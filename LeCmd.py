...
actdir = "/home/pi/AlienbotPi/ActionGroups/"
# 校准值
Calibration = (500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500)
...
def cmd_i001(par):  # 指令001 舵机运动
    ...
    for i in range(0, len(Servos), 2):
        if Servos[i] > 13 or Servos[i] < 1 or Servos[i+1] > 1000 or Servos[i+1] < 0:#Modify the number of steering gear
...
def cmd_i008(sock, data=[]):      # 修改偏差
    ...
    ########################################################
    if not len(Calibration) == 8 and not len(upper_d) == 8:
        print("偏差数量错误")
        s = "I008-dev-no\r\n"
